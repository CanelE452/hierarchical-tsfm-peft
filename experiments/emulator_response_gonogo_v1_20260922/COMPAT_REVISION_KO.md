...
## 5. 첫 실행 시도에서의 수정 (2026-09-22 22:35, optimizer update 0회)

- 증상: `stage_preflight_base`의 F_TOP3 무장부(journal-free) 검사에서 `LORA_MAPPING_UNKNOWN_KEYS`로 차단됐다. 25개 모듈에만 adapter가 붙은 모델에 97개 모듈 전체의 canonical 초기값을 넣었기 때문이다. 본학습(`stage_fit`)은 이미 모듈 목록으로 초기값을 거르고 있었다.
- 영향: smoke 장부가 생성되기 전(모든 update 이전)에 멈췄다. 소비한 optimizer update는 0회다.
- 수정: 같은 필터를 사전검사에도 적용했다(stages.py 한 줄). 첫 시도의 봉인·단계 기록은 지우고, 수정본으로 다시 봉인해 처음부터 재실행한다. 자료·설정·판정 기준은 그대로다.
