"""Statistical baseline predictor, copied verbatim from the official GIFT-Eval notebook
notebooks/naive.ipynb (cell 2) so that the SNAIVE arm matches the leaderboard construction:
quantiles come from statsforecast prediction intervals, multivariate sources are evaluated
as univariate. The only addition is the SeasonalNaivePredictor subclass at the end."""
    ...
    **model_params
    ...
    def sf_predict(self, Y_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.config.intervals is not None:
            # statsforecast >= 2.0 rejects level 0, which the official ModelConfig produces for the
            # 0.5 quantile. Level 0 is the point forecast, so it is dropped here and substituted
            # from the model column in yield_forecast; the resulting quantiles are unchanged.
            kwargs["level"] = [lv for lv in self.config.intervals if lv > 0]
    ...
    def yield_forecast(
        ...
        for idx in item_ids:
            ...
            for key in self.config.statsforecast_keys:
                if key == "mean" or key.endswith("-0"):
                    # "lo-0"/"hi-0" is the 0.5 quantile, which equals the point forecast
...
class SeasonalNaivePredictor(StatsForecastPredictor):
    """statsforecast SeasonalNaive, the model behind the leaderboard's seasonal_naive row."""
